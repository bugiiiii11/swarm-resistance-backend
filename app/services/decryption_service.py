# services/decryption_service.py - FIXED with proper newline handling
import base64
import os
import logging
...
class MedaShooterDecryption:
    ...
    def _create_temp_env_file(self):
        """Create PEM files with RAW content and explicit newlines"""
        
        # Use explicit \n characters and proper formatting
        score_key_lines = [
        ...
        score_key_pem = '\n'.join(score_key_lines)
        info_key_pem = '\n'.join(info_key_lines)
        
        # Write files with explicit newlines
        try:
            with open("medashooter_score.pem", "w", newline='\n') as f:
                f.write(score_key_pem)
                
            with open("medashooter_info.pem", "w", newline='\n') as f:
                f.write(info_key_pem)
                
            logger.info("✅ PEM key files created with explicit newlines")
            
            # Debug: Check that newlines are preserved
            with open("medashooter_score.pem", "r") as f:
                written_content = f.read()
                logger.info(f"🔍 File newline count: {written_content.count(chr(10))}")
                logger.info(f"🔍 File contains \\n: {'\\n' in written_content}")
                logger.info(f"🔍 File line count: {len(written_content.splitlines())}")
            
        except Exception as e:
            logger.error(f"❌ Failed to create PEM key files: {e}")
            raise
    ...
    def _load_keys(self):
        ...
        try:
            ...
            logger.info(f"Score key content starts with: {score_content[:35]}...")
            logger.info(f"Info key content starts with: {info_content[:35]}...")
            
            # Debug: Check content properties
            logger.info(f"🔍 Score content has newlines: {chr(10) in score_content}")
            logger.info(f"🔍 Score content line count: {len(score_content.splitlines())}")
            
            # Import RSA keys directly
            try:
                ...
                logger.error(f"❌ Score key import failed: {score_error}")
                raise
            ...
            try:
                ...
                logger.error(f"❌ Info key import failed: {info_error}")
                raise
            
            # Success!
            logger.info("✅ RSA keys loaded successfully from PEM files")
